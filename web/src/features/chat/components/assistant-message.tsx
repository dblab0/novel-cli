...
export function AssistantMessage({
  ...
  const content = useMemo(() => {
    switch (message.variant) {
      ...
      case "book-selector":
        return renderBookSelectorMessage(message, onSendMessage);
      default:
...
const renderBookSelectorMessage = (
  message: LiveMessage,
  onSendMessage?: (text: string) => void,
) => {
  if (!message.content) {
    return (
      <MessageContent className={assistantContentClass}>
        <div className="text-sm text-muted-foreground">无法解析书籍列表</div>
      </MessageContent>
    );
  }
  let data: { books: string[]; current_book: string | null };
  try {
