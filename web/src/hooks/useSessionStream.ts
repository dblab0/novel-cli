  ...
  const processEvent = useCallback(
    (event: WireEvent, isReplay = false, rpcMessageId?: string | number) => {
      switch (event.type) {
        ...
        case "BookList": {
          const bookPayload = (event as BookListEvent).payload;
          const bookMessageId = getNextMessageId("assistant");
          upsertMessage({
            id: bookMessageId,
            role: "assistant",
            variant: "book-selector",
            turnIndex:
              turnCounterRef.current > 0
                ? turnCounterRef.current - 1
                : undefined,
            content: JSON.stringify({
              books: bookPayload.books,
              current_book: bookPayload.current_book,
            }),
            isStreaming: false,
          });
          break;
