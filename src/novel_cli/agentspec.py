...
SUPPORTED_AGENT_SPEC_VERSIONS = (DEFAULT_AGENT_SPEC_VERSION, "1.1")
...
class ToolValidator(BaseModel):
    ...
    tool: str = Field(description="目标工具名")
    require_overview_before_detail: bool = Field(
        default=False,
        description="当调用带限定参数时，必须先调用过一次概览模式",
    )
...
class LoopDetectionConfig(BaseModel):
    ...
    Attributes:
        threshold: 连续重复调用阈值，默认 5，最小值 2。
    """

    threshold: int = Field(default=5, ge=2, description="连续重复调用阈值")
...
class AgentSpec(BaseModel):
    ...
    tool_validators: list[ToolValidator] | None = Field(
        default=None, description="工具调用顺序验证规则"
    )
    loop_detection: LoopDetectionConfig | None = Field(
        default=None, description="循环检测配置"
    )
...
class ResolvedAgentSpec:
    ...
    tool_validators: list[ToolValidator]
...
def load_agent_spec(agent_file: Path) -> ResolvedAgentSpec:
    ...
    return ResolvedAgentSpec(
        ...
        tool_validators=agent_spec.tool_validators or [],
        loop_detection=agent_spec.loop_detection,
    )
...
def _load_agent_spec(agent_file: Path) -> AgentSpec:
    ...
    if agent_spec.extend:
        ...
        if agent_spec.tool_validators is not None:
            base_agent_spec.tool_validators = agent_spec.tool_validators
        if agent_spec.loop_detection is not None:
            base_agent_spec.loop_detection = agent_spec.loop_detection
