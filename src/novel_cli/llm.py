...
if TYPE_CHECKING:
    from novel_cli.config import Config, LLMModel, LLMProvider
...
def augment_provider_with_env_vars(provider: LLMProvider, model: LLMModel) -> dict[str, str]:
    ...
    match provider.type:
        case "kimi":
            if base_url := os.getenv("NOVEL_BASE_URL"):
                provider.base_url = base_url
                applied["NOVEL_BASE_URL"] = base_url
            if api_key := os.getenv("NOVEL_API_KEY"):
                provider.api_key = SecretStr(api_key)
                applied["NOVEL_API_KEY"] = "******"
            if model_name := os.getenv("NOVEL_MODEL_NAME"):
                model.model = model_name
                applied["NOVEL_MODEL_NAME"] = model_name
            if max_context_size := os.getenv("NOVEL_MODEL_MAX_CONTEXT_SIZE"):
                model.max_context_size = int(max_context_size)
                applied["NOVEL_MODEL_MAX_CONTEXT_SIZE"] = max_context_size
            if capabilities := os.getenv("NOVEL_MODEL_CAPABILITIES"):
                ...
                applied["NOVEL_MODEL_CAPABILITIES"] = capabilities
...
def _novel_default_headers(provider: LLMProvider) -> dict[str, str]:
    ...
    match provider.type:
        case "kimi":
            ...
            chat_provider = Kimi(
                ...
                default_headers=_novel_default_headers(provider),
            )

            gen_kwargs: Kimi.GenerationKwargs = {}
            if session_id:
                gen_kwargs["prompt_cache_key"] = session_id
            if temperature := os.getenv("NOVEL_MODEL_TEMPERATURE"):
                gen_kwargs["temperature"] = float(temperature)
            if top_p := os.getenv("NOVEL_MODEL_TOP_P"):
                gen_kwargs["top_p"] = float(top_p)
            if max_tokens := os.getenv("NOVEL_MODEL_MAX_TOKENS"):
        ...
        case "_scripted_echo":
            ...
            trace_value = os.getenv("NOVEL_SCRIPTED_ECHO_TRACE", "")
...
def _load_scripted_echo_scripts() -> list[str]:
    script_path = os.getenv("NOVEL_SCRIPTED_ECHO_SCRIPTS")
    if not script_path:
        raise ValueError("NOVEL_SCRIPTED_ECHO_SCRIPTS is required for _scripted_echo.")
