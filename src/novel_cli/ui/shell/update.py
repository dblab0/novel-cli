...
提供版本检测和自动更新功能，支持从远程服务器下载最新版本并安装。
"""
...
BASE_URL = "https://cdn.novel-cli.dev/binaries/novel-cli"
LATEST_VERSION_URL = f"{BASE_URL}/latest"
INSTALL_DIR = Path.home() / ".local" / "bin"

# 升级命令，在 toast 通知中显示。可被包装脚本覆盖
UPGRADE_COMMAND = "uv tool upgrade novel-cli"
...
def semver_tuple(version: str) -> tuple[int, int, int]:
    ...
    Returns:
        版本元组 (major, minor, patch)，若解析失败返回 (0, 0, 0)。
    """
    v = version.strip()
    if v.startswith("v"):
        v = v[1:]
...
def _detect_target() -> str | None:
    ...
    Returns:
        目标平台字符串（如 "x86_64-apple-darwin"），若不支持返回 None。
    """
...
async def _do_update(*, print: bool, check_only: bool) -> UpdateResult:
    ...
    async with new_client_session(timeout=download_timeout) as session:
        ...
        filename = f"novel-{latest_version}-{target}.tar.gz"
        download_url = f"{BASE_URL}/{latest_version}/{filename}"

        with tempfile.TemporaryDirectory(prefix="novel-cli-") as tmpdir:
            ...
            try:
                ...
                for root, _, files in os.walk(tmpdir):
                    if "novel" in files:
                        binary_path = os.path.join(root, "novel")
                        break
                if not binary_path:
                    logger.error("Binary 'novel' not found in archive.")
                    _print("[red]Binary 'novel' not found in archive.[/red]")
                    return UpdateResult.FAILED
            except Exception:
    ...
    _print("[yellow]Restart Novel CLI to use the new version.[/yellow]")
