# ACP Integration Notes (novel-cli)
...
- **Single-session server**: `NovelCLI.run_acp()` uses `ACP` -> `ACPServerSingleSession`.
  - Code: `src/novel_cli/app.py`, `src/novel_cli/ui/acp/__init__.py`.
  - Used when running CLI with `--acp` UI mode.
- **Multi-session server**: `acp_main()` runs `ACPServer` with `use_unstable_protocol=True`.
  - Code: `src/novel_cli/acp/__init__.py`, `src/novel_cli/acp/server.py`.
  - Exposed via the `novel acp` command in `src/novel_cli/cli/__init__.py`.
...
- `session/new`
  - Multi-session: creates a persisted `Session`, builds `NovelCLI`, stores `ACPSession`.
...
- `session/load`
  - Multi-session only: loads by `Session.find`, then builds `NovelCLI` and `ACPSession`.
...
- `session/set_mode` and `session/set_model` (no multi-mode/model switching in novel-cli).
