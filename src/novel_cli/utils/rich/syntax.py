...
提供自定义的 rich 语法高亮主题，用于代码展示和 diff 显示。
"""
...
NOVEL_ANSI_THEME_NAME = "novel-ansi"
"""自定义 ANSI 主题名称。"""

NOVEL_ANSI_THEME = ANSISyntaxTheme(
    {
...
"""Novel 自定义 ANSI 语法高亮主题。"""


def resolve_code_theme(theme: str | SyntaxTheme) -> str | SyntaxTheme:
    """解析代码主题。

    如果主题名称为 "novel-ansi"，则返回自定义的 Novel ANSI 主题。

    Args:
        theme: 主题名称字符串或 SyntaxTheme 对象。

    Returns:
        解析后的主题字符串或 SyntaxTheme 对象。
    """
    if isinstance(theme, str) and theme.lower() == NOVEL_ANSI_THEME_NAME:
        return NOVEL_ANSI_THEME
    return theme


class NovelSyntax(Syntax):
    """Novel 代码语法高亮类。

    继承自 rich.syntax.Syntax，默认使用 Novel ANSI 主题。
...
if __name__ == "__main__":
    ...
    for idx, (title, lexer, code) in enumerate(examples):
        ...
        console.print(NovelSyntax(code, lexer))