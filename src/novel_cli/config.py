...
from novel_cli.hooks.config import HookDef
from novel_cli.llm import ModelCapability, ProviderType
...
class NovelDBConfig(BaseModel):
    """Novel knowledge database connection configuration (unified PostgreSQL)."""

    # PostgreSQL - unified storage
    pg_host: str = "localhost"
    pg_port: int = 5433
    pg_db: str = "novel_graph_search"
    pg_user: str = "novel_admin"
    pg_password: SecretStr = SecretStr("")
    pg_pool_size: int = 15

    # 关系类型元数据目录（relationship_meta.json 所在目录的父目录）
    data_dir: str = ""
    ...
    @field_serializer("pg_password", "embedding_api_key", when_used="json")
    def dump_secret(self, v: SecretStr):
        return v.get_secret_value()
...
class Services(BaseModel):
    ...
    """Moonshot Fetch configuration."""
    novel_db: NovelDBConfig = Field(default_factory=NovelDBConfig)
    """Novel knowledge database configuration."""
...
class Config(BaseModel):
    ...
    merge_all_available_skills: bool = Field(
        default=False,
        description=(
            "Merge skills from all existing brand directories (novel/claude/codex) "
