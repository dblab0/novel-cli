...
from novel_cli.soul import StatusSnapshot
from novel_cli.ui.shell import prompt as shell_prompt
...
def _make_toolbar_session(*, model_name: str | None = None, tips: list[str] | None = None) -> Any:
    ...
    prompt_session._current_book_provider = None
...
# ── Book display in toolbar ───────────────────────────────────────────────────


def test_bottom_toolbar_shows_book_when_selected(monkeypatch: Any) -> None:
    """provider 返回书名时，toolbar 第 1 行包含 📖 书名。"""
    session = _make_toolbar_session()
    session._current_book_provider = lambda: "斗破苍穹"

    lines = _render_toolbar_lines(session, 80, monkeypatch)

    assert "📖 斗破苍穹" in lines[1]


def test_bottom_toolbar_hides_book_when_none(monkeypatch: Any) -> None:
    """provider 返回 None 时，toolbar 不包含 📖。"""
    session = _make_toolbar_session()
    session._current_book_provider = lambda: None
...
@pytest.mark.parametrize("width", [40, 60, 80])
def test_bottom_toolbar_never_overflows_with_book(width: int, monkeypatch: Any) -> None:
    """有书时 toolbar 仍然不会溢出。"""
    session = _make_toolbar_session(
        model_name="kimi-latest",
        tips=["ctrl-x: toggle mode"],
    )
    session._current_book_provider = lambda: "很长的书名用于测试窄终端降级行为"
    session._background_task_count_provider = lambda: 2

    lines = _render_toolbar_lines(
        session,
        width,
        monkeypatch,
        git_branch="feature/long-branch",
        git_status_result=(True, 3, 1),
        cwd="~/src/project/subdir",
    )
