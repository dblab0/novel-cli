...
from novel_cli.config import LLMProvider
...
from novel_cli.ui.shell import slash as shell_slash
...
def _setup_feedback_provider(runtime: Runtime) -> None:
    """Add a managed:kimi-code provider to the runtime config."""
...
def _setup_submission_mocks(monkeypatch, *, feedback_text="Great tool!", session_factory=None):
    ...
    if session_factory is not None:
        monkeypatch.setattr("novel_cli.utils.aiohttp.new_client_session", session_factory)
...
class TestFeedbackGuards:
    async def test_fallback_when_no_novel_soul(self, monkeypatch) -> None:
        """When soul is not NovelSoul, should fallback to GitHub issues."""
        shell = Mock()
        shell.soul = Mock()  # not spec=NovelSoul
    ...
    async def test_fallback_when_no_managed_provider(
        self, runtime: Runtime, tmp_path: Path, monkeypatch
    ) -> None:
        """When the managed platform is not available, should fallback."""
        runtime.config.providers["managed:kimi-code"] = LLMProvider(
            type="kimi",
            base_url="https://api.kimi.com/coding/v1",
            api_key=SecretStr("test-api-key"),
        ...
        open_mock = Mock(return_value=True)
        monkeypatch.setattr("webbrowser.open", open_mock)
        monkeypatch.setattr("novel_cli.auth.platforms.get_platform_by_id", lambda _: None)
...
class TestFeedbackUserInput:
    ...
    async def test_cancelled_on_interrupt(
        ...
        assert any("已取消" in str(call) for call in print_mock.call_args_list)
    ...
    async def test_empty_feedback_rejected(
        ...
        assert any("不能为空" in str(call) for call in print_mock.call_args_list)
...
class TestFeedbackSubmission:
    async def test_successful_submission(
        ...
        assert any("已提交" in str(call) for call in print_mock.call_args_list)
    ...
    async def test_timeout_fallback(self, runtime: Runtime, tmp_path: Path, monkeypatch) -> None:
        ...
        assert any("超时" in str(call) for call in print_mock.call_args_list)
        open_mock.assert_called_once()
    ...
    async def test_network_error_fallback(
        ...
        assert any("网络错误" in str(call) for call in print_mock.call_args_list)
        open_mock.assert_called_once()
