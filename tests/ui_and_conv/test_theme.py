...
from novel_cli.config import Config, get_default_config
...
from novel_cli.ui.theme import (
    get_active_theme,
    get_diff_colors,
...
def test_theme_command_registered_in_both_registries():
    from novel_cli.ui.shell.slash import registry, shell_mode_registry
...
def test_theme_invalid_arg(runtime: Runtime, tmp_path: Path, monkeypatch):
    ...
    assert "未知主题" in str(print_mock.call_args.args[0])
...
def test_theme_same_as_current(runtime: Runtime, tmp_path: Path, monkeypatch):
    ...
    assert "已在使用" in str(print_mock.call_args.args[0])
...
def test_theme_rejects_inline_config(runtime: Runtime, tmp_path: Path, monkeypatch):
    ...
    assert "配置文件" in str(print_mock.call_args.args[0])
...
def test_theme_save_failure_no_reload_no_state_change(
    ...
    assert "保存配置失败" in str(print_mock.call_args.args[0])
    assert get_active_theme() == "dark"
...
async def test_shell_startup_initializes_theme_from_config(
    ...
    from novel_cli.ui import theme as theme_mod
    from novel_cli.ui.shell import Shell as RealShell
