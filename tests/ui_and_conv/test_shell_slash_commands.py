...
from novel_cli.cli import Reload
from novel_cli.session import Session
from novel_cli.ui.shell.slash import ShellSlashCmdFunc, shell_mode_registry
...
def isolated_share_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    monkeypatch.setattr("novel_cli.share.get_share_dir", _get_share_dir)
    monkeypatch.setattr("novel_cli.metadata.get_share_dir", _get_share_dir)
    return share_dir
...
def mock_shell(work_dir: KaosPath) -> Mock:
    """Create a mock Shell whose soul passes the NovelSoul isinstance check.
...
class TestNewCommandRegistration:
    ...
    def test_registered_in_shell_registry(self) -> None:
        ...
        assert cmd.name == "new"
        assert cmd.description.startswith("开始新会话")
...
class TestNewCommandBehavior:
    ...
    async def test_returns_early_without_novel_soul(self) -> None:
        """When soul is not a NovelSoul, the command should silently return."""
        shell = Mock()
        shell.soul = Mock()  # plain Mock, not spec=NovelSoul
