...
from novel_cli.subagents import AgentLaunchSpec
from novel_cli.tools.display import ShellDisplayBlock
from novel_cli.ui.shell import Shell
from novel_cli.ui.shell import slash as shell_slash
from novel_cli.wire.types import ApprovalRequest
...
async def test_task_command_rejects_args(runtime: Runtime, tmp_path: Path, monkeypatch) -> None:
    ...
    assert '用法: "/task"' in str(print_mock.call_args.args[0])
...
async def test_task_command_requires_root_role(
    ...
    print_mock.assert_called_once()
    assert "根智能体" in str(print_mock.call_args.args[0])
