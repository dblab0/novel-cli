...
def _novel_cli_bin() -> str:
    """Return the path to the novel-cli entry-point script inside the venv."""
    return str(_repo_root() / ".venv" / "bin" / "novel-cli")
...
def acp_share_dir(tmp_path: Path) -> Path:
    ...
    trace_env = os.getenv("NOVEL_SCRIPTED_ECHO_TRACE", "0")
    config_data = {
        ...
        "providers": {
            "scripted_provider": {
                "type": "_scripted_echo",
                "base_url": "",
                "api_key": "test-api-key-for-ci",  # 非空 API key 以通过 _check_auth
                "env": {
                    "NOVEL_SCRIPTED_ECHO_SCRIPTS": str(scripts_path),
                    "NOVEL_SCRIPTED_ECHO_TRACE": trace_env,
                },
            }
        },
    }
    ...
    (credentials_dir / "novel-cli.json").write_text(
        json.dumps(
            {
...
async def acp_client(
    ...
    """Spawn a novel-cli ACP subprocess and return the SDK connection + test client."""
    test_client = ACPTestClient()
    env = {
        **os.environ,
        "NOVEL_SHARE_DIR": str(acp_share_dir),
    }
