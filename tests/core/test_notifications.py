...
from novel_cli.approval_runtime import ApprovalSource
from novel_cli.background import TaskRuntime, TaskSpec
from novel_cli.llm import LLM
from novel_cli.notifications import NotificationEvent
from novel_cli.soul import StatusSnapshot, _current_wire, run_soul
...
from novel_cli.wire.types import ApprovalRequest, ApprovalResponse, Notification
...
def _make_soul(runtime: Runtime, tmp_path: Path) -> tuple[NovelSoul, Context]:
    llm = LLM(
    ...
    return NovelSoul(agent, context=context), context
...
@pytest.mark.asyncio
async def test_novelsoul_appends_notification_message(runtime: Runtime, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_novel_cli_run_yields_root_hub_approvals(runtime: Runtime) -> None:
    ...
    cli = NovelCLI(_ApprovalOnlySoul(runtime), runtime, {})  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_novel_cli_run_bridges_approval_resolution_back_to_runtime(runtime: Runtime) -> None:
    ...
    cli = NovelCLI(soul, runtime, {})  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_novel_cli_run_replays_pending_approvals_from_previous_turn(runtime: Runtime) -> None:
    assert runtime.approval_runtime is not None
