...
from novel_cli.exception import InvalidToolError, SystemPromptTemplateError
from novel_cli.session import Session
from novel_cli.soul.agent import BuiltinSystemPromptArgs, Runtime, _load_system_prompt, load_agent
...
def test_load_system_prompt(system_prompt_file: Path, builtin_args: BuiltinSystemPromptArgs):
    ...
    assert builtin_args.NOVEL_NOW in prompt
...
def test_system_prompt_contains_platform_info(builtin_args: BuiltinSystemPromptArgs):
    ...
    assert builtin_args.NOVEL_OS in prompt
    assert builtin_args.NOVEL_SHELL in prompt
...
def test_load_system_prompt_allows_literal_dollar(builtin_args: BuiltinSystemPromptArgs):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        system_md.write_text("Price is $100, path $PATH, time ${NOVEL_NOW}.")
...
def test_load_tools_valid(runtime: Runtime):
    ...
    tool_paths = ["novel_cli.tools.think:Think", "novel_cli.tools.shell:Shell"]
...
def test_load_tools_invalid(runtime: Runtime):
    ...
    tool_paths = ["novel_cli.tools.nonexistent:Tool", "novel_cli.tools.think:Think"]
    toolset = NovelToolset()
    try:
        toolset.load_tools(
            tool_paths,
        ...
        assert "novel_cli.tools.nonexistent:Tool" in str(e)
...
async def test_load_agent_invalid_tools(agent_file_invalid_tools: Path, runtime: Runtime):
    ...
    with pytest.raises(ValueError, match="无效工具"):
...
async def test_load_agent_starts_mcp_in_background(runtime: Runtime, monkeypatch):
    ...
    monkeypatch.setattr(NovelToolset, "load_mcp_tools", fake_load_mcp_tools)
...
async def test_load_agent_can_defer_mcp_loading(runtime: Runtime, monkeypatch):
    ...
    monkeypatch.setattr(NovelToolset, "load_mcp_tools", fake_load_mcp_tools)
    monkeypatch.setattr(NovelToolset, "defer_mcp_tool_loading", fake_defer_mcp_tool_loading)
...
agent:
  name: "Test Agent"
  system_prompt_path: ./system.md
  tools: ["novel_cli.tools.nonexistent:Tool"]
""")
...
def system_prompt_file() -> Generator[Path, Any, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        system_md = tmpdir / "system.md"
        system_md.write_text("Test system prompt with ${NOVEL_NOW} and ${CUSTOM_ARG}")
