"""Tests for --plan flag and default_plan_mode config in NovelCLI.create()."""
...
import novel_cli.app as app_module
from novel_cli.app import NovelCLI

# ---------------------------------------------------------------------------
# Helpers — lightweight fakes for NovelCLI.create() dependencies
# ---------------------------------------------------------------------------


def _patch_create_deps(monkeypatch, *, session_plan_mode: bool = False):
    """Patch heavy dependencies so NovelCLI.create() runs without I/O.

    Returns a FakeSoul class whose instances record plan-mode interactions.
    """

    class FakeSoul:
        """Tracks plan-mode calls made by NovelCLI.create()."""
    ...
    async def fake_runtime_create(config, _llm, session, yolo, **kwargs):
    ...
    monkeypatch.setattr(app_module, "NovelSoul", FakeSoul)
...
class TestPlanFlagNewSession:
    ...
    async def test_plan_flag_activates_plan_mode(self, session, config, monkeypatch):
        ...
        await NovelCLI.create(session, config=config, plan_mode=True, resumed=False)
    ...
    async def test_config_default_plan_mode_activates(self, session, config, monkeypatch):
        ...
        await NovelCLI.create(session, config=config, plan_mode=False, resumed=False)
...
class TestPlanFlagResumedSession:
    ...
    async def test_config_not_applied_on_resumed_session(self, session, config, monkeypatch):
        ...
        await NovelCLI.create(session, config=config, plan_mode=False, resumed=True)
    ...
    async def test_plan_flag_activates_on_resumed_session_without_plan(
        ...
        await NovelCLI.create(session, config=config, plan_mode=True, resumed=True)
...
class TestPlanFlagPriority:
    ...
    async def test_default_is_false(self, session, config, monkeypatch):
        ...
        await NovelCLI.create(session, config=config, resumed=False)
    ...
    async def test_plan_and_yolo_coexist(self, session, config, monkeypatch):
        ...
        await NovelCLI.create(session, config=config, plan_mode=True, yolo=True, resumed=False)
...
class TestSchedulePlanActivationReminder:
    """Unit tests for NovelSoul.schedule_plan_activation_reminder()."""
    ...
    def test_schedules_when_in_plan_mode(self, runtime, tmp_path, monkeypatch):
        ...
        from novel_cli.soul.agent import Agent
        ...
        soul = NovelSoul(agent, context=Context(file_backend=tmp_path / "ctx.jsonl"))
...
class TestWebWorkerResumedDetection:
    ...
    async def test_new_session_without_state_file_is_not_resumed(self, tmp_path):
        ...
        fake_joint = SimpleNamespace(novel_cli_session=fake_session)

        with (
            patch("novel_cli.web.runner.worker.load_session_by_id", return_value=fake_joint),
            patch(
                "novel_cli.web.runner.worker.get_global_mcp_config_file",
                return_value=tmp_path / "no-mcp.json",
            ),
            patch.object(NovelCLI, "create", side_effect=spy_create),
