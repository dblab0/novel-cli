...
def test_install_plugin(tmp_path: Path):
    ...
    assert pj["runtime"]["host"] == "novel-cli"
...
async def test_skill_discovery_includes_plugins_dir(tmp_path: Path, monkeypatch):
    ...
    from novel_cli.skill import resolve_skills_roots
    ...
    # Point NOVEL_SHARE_DIR to tmp_path so get_plugins_dir() returns tmp_path/plugins
    monkeypatch.setenv("NOVEL_SHARE_DIR", str(tmp_path))
...
def _make_config(*, api_key: str = "sk-test"):
...
def test_collect_host_values_static_key():
    """Static API key is returned correctly."""
    config = _make_config(api_key="sk-static-key")

    values = collect_host_values(config)
