...
from novel_cli.tools.plan import ExitPlanMode, Params, PlanOption
from novel_cli.tools.plan.enter import _DESCRIPTION, EnterPlanMode
from novel_cli.tools.plan.heroes import (
    _slug_cache,
    get_or_create_slug,
    get_plan_file_path,
    read_plan_file,
...
from novel_cli.wire.types import QuestionNotSupported, QuestionRequest, ToolCall
...
def _make_soul(runtime: Runtime, tmp_path: Path) -> NovelSoul:
    agent = Agent(
        name="Test Agent",
    ...
    return NovelSoul(agent, context=Context(file_backend=tmp_path / "history.jsonl"))
...
class TestGetOrCreateSlug:
    def test_returns_hero_name(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("novel_cli.tools.plan.heroes.PLANS_DIR", tmp_path)
    ...
    def test_collision_fallback(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("novel_cli.tools.plan.heroes.HERO_NAMES", ["a", "b"])
...
def _mock_wire_and_tool_call(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("novel_cli.tools.plan.get_wire_or_none", lambda: wire_mock)
    monkeypatch.setattr("novel_cli.tools.plan.wire_send", lambda msg: None)
    monkeypatch.setattr("novel_cli.tools.plan.enter.get_wire_or_none", lambda: wire_mock)
    monkeypatch.setattr("novel_cli.tools.plan.enter.wire_send", lambda msg: None)
    tc = ToolCall(id="test-tc", function=ToolCall.FunctionBody(name="ExitPlanMode", arguments=None))
    monkeypatch.setattr("novel_cli.tools.plan.get_current_tool_call_or_none", lambda: tc)
    monkeypatch.setattr("novel_cli.tools.plan.enter.get_current_tool_call_or_none", lambda: tc)
    return wire_mock
...
class TestExitPlanModeHappyPaths:
    ...
    async def test_wire_unavailable(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        tool, _, _ = _setup_exit_tool(tmp_path)
        monkeypatch.setattr("novel_cli.tools.plan.get_wire_or_none", lambda: None)
...
class TestEnterPlanModeHappyPaths:
    ...
    async def test_wire_unavailable(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        tool, _, _ = _setup_enter_tool(tmp_path)
        monkeypatch.setattr("novel_cli.tools.plan.enter.get_wire_or_none", lambda: None)
...
# ---------------------------------------------------------------------------
# NovelSoul — plan mode state management
# ---------------------------------------------------------------------------


class TestNovelSoulPlanState:
...
# ---------------------------------------------------------------------------
# NovelSoul — plan_session_id cross-process persistence
# ---------------------------------------------------------------------------


class TestNovelSoulPlanSessionPersistence:
    """Tests that plan_session_id survives simulated process restarts."""

    async def test_plan_session_id_survives_restart(
        self, runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Second NovelSoul created from same session state gets same plan file path."""
        monkeypatch.setattr("novel_cli.tools.plan.heroes.PLANS_DIR", tmp_path)
        ...
        # New NovelSoul reads from same (already-saved) session state and re-seeds cache
    ...
    async def test_plan_session_id_cleared_after_deactivation(
        ...
        # Restart + re-activate
        from novel_cli.tools.plan.heroes import _slug_cache
