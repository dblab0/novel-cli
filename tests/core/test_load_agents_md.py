...
from novel_cli.soul.agent import _AGENTS_MD_MAX_BYTES, load_agents_md
...
async def test_novel_dir_and_root_both_loaded(temp_work_dir: KaosPath):
    """.novel/AGENTS.md and AGENTS.md in the same dir are both loaded; .novel/ first."""
    novel_dir = temp_work_dir / ".novel"
    await novel_dir.mkdir()
    await (novel_dir / "AGENTS.md").write_text("novel agents")
    await (temp_work_dir / "AGENTS.md").write_text("root agents")

    content = await load_agents_md(temp_work_dir)

    assert content is not None
    assert content.index("novel agents") < content.index("root agents")
    assert content.count("<!-- From:") == 2


async def test_novel_dir_in_parent(temp_work_dir: KaosPath):
    """.novel/AGENTS.md in a parent directory is discovered via hierarchy."""
    await (temp_work_dir / ".git").mkdir()
    novel_dir = temp_work_dir / ".novel"
    await novel_dir.mkdir()
    await (novel_dir / "AGENTS.md").write_text("parent novel")
    ...
    assert content is not None
    assert "parent novel" in content
    assert "child root" in content
    assert content.index("parent novel") < content.index("child root")
