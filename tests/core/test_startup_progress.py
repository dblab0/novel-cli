...
import novel_cli.ui.shell.startup as startup_module
...
@pytest.mark.asyncio
async def test_novel_cli_create_reports_startup_phases(session, config, monkeypatch) -> None:
    phases: list[str] = []
    ...
    monkeypatch.setattr(app_module, "NovelSoul", _FakeSoul)

    cli = await NovelCLI.create(session, config=config, startup_progress=phases.append)

    assert isinstance(cli, NovelCLI)
    assert phases == [
...
@pytest.mark.asyncio
async def test_novel_cli_create_cleans_stale_running_foreground_subagents(
    ...
    await NovelCLI.create(session, config=config)
