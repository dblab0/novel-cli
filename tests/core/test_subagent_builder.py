...
from novel_cli.soul.agent import load_agent
from novel_cli.subagents.builder import SubagentBuilder
from novel_cli.subagents.models import AgentLaunchSpec, AgentTypeDefinition, ToolPolicy
...
async def test_builder_model_priority_prefers_override_then_type_default_then_inherit(
    ...
    def fake_clone_llm_with_model_alias(llm, config, model_alias, *, session_id):
        captured_aliases.append(model_alias)
        return llm

    monkeypatch.setattr(
        "novel_cli.subagents.builder.clone_llm_with_model_alias",
