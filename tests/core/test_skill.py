...
async def test_resolve_skills_roots_uses_layers(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NOVEL_SHARE_DIR", str(tmp_path / "share"))
...
async def test_find_user_skills_dirs_empty_generic_does_not_shadow_brand(monkeypatch, tmp_path):
    """Core bug: empty ~/.config/agents/skills should NOT hide ~/.novel/skills."""
    home_dir = tmp_path / "home"
    generic_dir = home_dir / ".config" / "agents" / "skills"
    generic_dir.mkdir(parents=True)  # exists but empty

    brand_dir = home_dir / ".novel" / "skills"
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_brand_group_prefers_novel_over_claude(monkeypatch, tmp_path):
    """Brand group: ~/.novel/skills takes priority over ~/.claude/skills."""
    home_dir = tmp_path / "home"
    novel_dir = home_dir / ".novel" / "skills"
    ...
    dirs = await find_user_skills_dirs()
    # Only novel should be selected (first existing in brand group)
    assert KaosPath.unsafe_from_local_path(novel_dir) in dirs
...
async def test_find_project_skills_dirs_merge(tmp_path):
    ...
    generic_dir.mkdir(parents=True)
    brand_dir = work_dir / ".novel" / "skills"
    brand_dir.mkdir(parents=True)
...
@pytest.mark.asyncio
async def test_find_project_skills_dirs_brand_prefers_novel(tmp_path):
    """Project layer brand group: .novel/skills wins over .claude/skills."""
    work_dir = tmp_path / "project"
    novel_dir = work_dir / ".novel" / "skills"
    novel_dir.mkdir(parents=True)
    ...
    dirs = await find_project_skills_dirs(KaosPath.unsafe_from_local_path(work_dir))
    assert len(dirs) == 1
    assert dirs[0] == KaosPath.unsafe_from_local_path(novel_dir)
...
async def test_resolve_skills_roots_merges_user_and_project(monkeypatch, tmp_path):
    ...
    user_brand = home_dir / ".novel" / "skills"
    ...
    proj_brand = work_dir / ".novel" / "skills"
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_novel_and_claude(monkeypatch, tmp_path):
    """merge_brands=True: novel + claude both exist → both returned, novel first."""
    home_dir = tmp_path / "home"
    novel_dir = home_dir / ".novel" / "skills"
    novel_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    ...
    # novel before claude
    novel_idx = dirs.index(KaosPath.unsafe_from_local_path(novel_dir))
    claude_idx = dirs.index(KaosPath.unsafe_from_local_path(claude_dir))
    assert novel_idx < claude_idx


@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_all_three(monkeypatch, tmp_path):
    """merge_brands=True: all three brand dirs → [novel, claude, codex]."""
    home_dir = tmp_path / "home"
    novel_dir = home_dir / ".novel" / "skills"
    novel_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    ...
    assert brand_dirs[0] == KaosPath.unsafe_from_local_path(novel_dir)
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_same_skill_novel_wins(monkeypatch, tmp_path):
    """merge_brands=True + same skill name → novel version wins via discover."""
    home_dir = tmp_path / "home"
    novel_dir = home_dir / ".novel" / "skills"
    novel_dir.mkdir(parents=True)
    _write_skill(
        novel_dir / "deploy",
        "---\nname: deploy\ndescription: novel deploy\n---\n",
    )
    claude_dir = home_dir / ".claude" / "skills"
    claude_dir.mkdir(parents=True)
    _write_skill(
    ...
    skills = await discover_skills_from_roots(dirs)
    assert len(skills) == 1
    assert skills[0].description == "novel deploy"
...
async def test_resolve_skills_roots_passes_merge_brands(monkeypatch, tmp_path):
    ...
    # Without merge_brands: only novel
    roots_default = await resolve_skills_roots(
        KaosPath.unsafe_from_local_path(work_dir),
    )
    assert KaosPath.unsafe_from_local_path(novel_dir) in roots_default
    ...
    assert KaosPath.unsafe_from_local_path(novel_dir) in roots_merged
