...
from novel_cli.tools.web.fetch import FetchURL, Params
...
async def test_fetch_url_basic_functionality(fetch_url_tool: FetchURL) -> None:
    ...
    # 网络超时时跳过测试（环境问题）
    if result.is_error and "timed out" in result.message:
        pytest.skip("Network timeout - external URL not accessible in this environment")

    assert not result.is_error
    # 百度页面内容可能变化，只检查基本内容存在
    output_str = str(result.output)
    assert "百度" in output_str or "baidu" in output_str.lower()
...
async def test_fetch_url_404_url(fetch_url_tool: FetchURL) -> None:
    ...
    result = await fetch_url_tool(
        Params(url="https://www.baidu.com/nonexistent-page-404-test")
    )
    ...
    if "timed out" in result.message:
        pytest.skip("Network timeout - external URL not accessible in this environment")
    # 百度对不存在的页面可能返回 200 或 404，检查错误消息格式
...
async def test_fetch_url_with_service(runtime) -> None:
    ...
    from novel_cli.config import Config, MoonshotFetchConfig, Services
