...
from novel_cli.tools.shell import Params, Shell
from novel_cli.tools.utils import DEFAULT_MAX_CHARS
...
async def test_command_with_error(shell_tool: Shell):
    ...
    # 支持中英文错误信息：英文 "No such file or directory"，中文 "没有那个文件或目录"
    assert ("No such file or directory" in result.output or "没有那个文件或目录" in result.output)
...
async def test_timeout_parameter_validation_bounds(shell_tool: Shell):
    ...
    from novel_cli.tools.shell import MAX_BACKGROUND_TIMEOUT, MAX_FOREGROUND_TIMEOUT
...
async def test_cancelled_command_kills_process(shell_tool: Shell, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("novel_cli.tools.shell.kaos.exec", fake_exec)
