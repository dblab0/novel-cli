        ...
        json.dumps(
            {
                ...
                "providers": {
                    "root_provider": {
                        ...
                        "base_url": "",
                        "api_key": "",
                        "env": {"NOVEL_SCRIPTED_ECHO_SCRIPTS": str(root_scripts_path)},
                    },
                    "sub_provider": {
                        "type": "_scripted_echo",
                        "base_url": "",
                        "api_key": "",
                        "env": {"NOVEL_SCRIPTED_ECHO_SCRIPTS": str(sub_scripts_path)},
                    },
                },
            }
        ),
...
def test_shell_session_resume_and_replay(tmp_path: Path) -> None:
    ...
    try:
        first_shell.read_until_contains("欢迎来到 Novel CLI!")
    ...
    try:
        second_shell.read_until_contains("欢迎来到 Novel CLI!")
...
def test_shell_clear_reloads_without_replaying_old_turns(tmp_path: Path) -> None:
    ...
    try:
        ...
        shell.read_until_contains("欢迎来到 Novel CLI!", after=clear_mark)
...
def test_shell_cancel_running_command_kills_process_and_recovers(tmp_path: Path) -> None:
    ...
    try:
        ...
        shell.read_until_contains("被用户中断", after=cancel_mark)
