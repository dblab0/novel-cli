...
## Goals and Scope
- Test only `novel --wire` JSON-RPC + wire messages; no Shell UI/Print/ACP/Term/shortcuts.
- Do not test: W-23, W-26, W-29, W-27 (env overrides).

## Execution Rules
- Tests run via `uv run novel` by default; set `NOVEL_E2E_WIRE_CMD` to override the base command
  (e.g. `../novel-agent-rs/target/debug/novel-agent` or `novel-agent` on PATH). `--wire` is appended if missing.
- Always isolate `HOME`, `USERPROFILE`, and `NOVEL_SHARE_DIR`, and use a temporary `--work-dir` to
  avoid touching real `~/.novel`.
