...
def close_replaced_openai_client(client: AsyncOpenAI, *, client_kwargs: Mapping[str, Any]) -> None:
    """
    关闭被替换的 OpenAI 客户端，除非这会关闭共享的外部 http 客户端。

    当调用者向 `AsyncOpenAI` 传递 `http_client=...` 时，多个包装器可能共享同一个
    `httpx.AsyncClient`。关闭被替换的包装器也会关闭该共享客户端，并立即破坏
    新包装器。
    """
...
def convert_error(error: OpenAIError | httpx.HTTPError) -> ChatProviderError:
    # httpx 错误可能在流式传输期间从 OpenAI SDK 泄漏出来；
    # 委托给共享转换器。
    if isinstance(error, httpx.HTTPError):
        return convert_httpx_error(error)
    # OpenAI SDK 错误 — 在父类之前检查子类以避免
    # 误分类（例如 APITimeoutError 继承 APIConnectionError）。
    match error:
        ...
        case openai.APIError() if type(error) is openai.APIError and error.body is None:
            # 没有主体的基础 APIError 表示传输层故障
            # （例如流式传输期间的"网络连接丢失"）。来自服务器的
            # SSE 错误事件携带主体字典，应该落入默认情况。
...
def _classify_base_api_error(message: str) -> ChatProviderError:
    """启发式地将 ``openai.APIError`` 消息映射到可重试错误类型。

    首先检查超时模式，因为像"连接超时"这样的消息应该分类为超时，
    而不是连接错误。
    """
...
def thinking_effort_to_reasoning_effort(effort: ThinkingEffort) -> ReasoningEffort:
    match effort:
        ...
        case "high":
            return "high"
        case "xhigh":
            # OpenAI 原生支持 gpt-5.1-codex-max 之后模型的 xhigh。
            return "xhigh"
        case "max":
            # OpenAI 的上限是 xhigh；kosong 的 ``max``（Anthropic 特有）
            # 限制为 OpenAI 接受的最高级别，而不是降到 high。
            return "xhigh"
...
def tool_to_openai(tool: Tool) -> ChatCompletionToolParam:
    """将单个工具转换为 OpenAI 工具格式。"""
    # 简单地使用 `model_dump`，因为 `Tool` 类型与 OpenAI 兼容
    return {
        "type": "function",
        "function": {
            "name": tool.name,
