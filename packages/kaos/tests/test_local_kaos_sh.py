"""通过 /bin/sh -c 运行命令的 `kaos.exec` 测试。"""
...
pytestmark = pytest.mark.skipif(
    platform.system() == "Windows", reason="/bin/sh 在 Windows 上不可用。"
)
...
async def run_sh(
    ...
    """通过 kaos.exec 执行 shell 命令并收集退出码和流输出。"""
...
async def test_command_with_error():
    ...
    assert ("No such file or directory" in stderr or "没有那个文件或目录" in stderr)
